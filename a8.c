...
int reg_counter = 0; 
RegisterMap reg_map[10]; // Array to store variable and constant-register 
mappings 
...
void translate_to_assembly(const char *line, FILE *output_file) { 
    ...
    if (strchr(clean_line, ':') && !strstr(clean_line, "=") && 
!strstr(clean_line, "goto")) { 
    ...
    if (strstr(clean_line, "if")) { 
        ...
        const char *reg2 = isdigit(var2[0]) ? get_register(var2, output_file) 
: get_register(var2, output_file); 
 
        // Output the CMP instruction and appropriate jump based on the 
condition 
    ...
    if (strstr(clean_line, "=")) { 
        ...
        if (strstr(expr, "+")) { 
            ...
            // Otherwise, assume it's a variable and move it to the target 
register 
...
int main() { 
    FILE *input_file = fopen("code.txt", "r"); 
...
fclose(input_file); 
fclose(output_file); 
printf("Translation completed! Assembly code saved in 
assembly_code.asm.\n"); 
return 0; 
}
